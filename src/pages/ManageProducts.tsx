...
const ManageProducts = () => {
  ...
  const handleEnrichProducts = async () => {
    try {
      ...
      const successCount = (data.results as EnrichResult[])?.filter(r => r.status === 'success').length || 0;
  ...
  const handleGenerateAIImages = async () => {
    try {
      ...
      const successCount = (data.results as GenerateResult[])?.filter(r => r.status === 'success').length || 0;
