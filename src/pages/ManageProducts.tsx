...
const ManageProducts = () => {
  ...
  const handleEnrichProducts = async () => {
    try {
      ...
      const successCount = (data.results as EnrichResult[] | undefined)?.filter((r) => r.status === 'success').length || 0;
  ...
  const handleGenerateAIImages = async () => {
    try {
      ...
      const successCount = (data.results as AIGenerationResult[] | undefined)?.filter((r) => r.status === 'success').length || 0;
