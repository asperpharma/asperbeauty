...
// Map queue item status to product status
function mapQueueStatus(status: "queued" | "processing" | "completed" | "failed" | "retrying"): "pending" | "processing" | "completed" | "failed" {
  switch (status) {
    case "queued":
      return "pending";
    case "retrying":
      ...
      setProducts(prev => 
        prev.map(p => {
          ...
          if (queueItem) {
            return {
              ...p,
              status: mapQueueStatus(queueItem.status),
