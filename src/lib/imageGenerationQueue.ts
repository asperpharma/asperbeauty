...
type QueueEventCallback = (data: QueueItem | QueueStats | Record<string, unknown>) => void;
...
class ImageGenerationQueue {
  ...
  private emit(event: QueueEventType, data: QueueItem | QueueStats | Record<string, unknown>) {
    const listeners = this.eventListeners.get(event);
    if (listeners) {
